...
namespace cercle17
{
    public partial class frmEXPORT_CPLUS_FraPro : Form
    {
        public frmEXPORT_CPLUS_FraPro()
        {
            InitializeComponent();
        }
        ...
        private void Cargar()
        {
            ...
            string strQ = "SELECT FC.Factura,FC.Anyo,FC.DetCod, D.DetNom, D.SubCta,FC.FechaEmision,FC.ImpteFactura,FC.ImptePendiente,FC.ImpteCobrado,FC.Serie,FC.BI1,FC.IVA1,FC.RE1 FROM FACTV_CABE FC, DETALLISTAS D WHERE (FC.DetCod=D.DetCod) ";
            ...
            try
            {
                ...
                while (myReader.Read())
                {
                    ...
                    factura.SubCta = myReader["SubCta"].ToString();
                    factura.BI = myReader["BI1"].ToString();
                    factura.IVA = myReader["IVA1"].ToString();
                    factura.Req = myReader["RE1"].ToString();
                ...
                if (dataGridView_Facturado.Rows.Count > 0)
                {
                    ...
                    dataGridView_Facturado.Columns[10].Visible = false;                                         //SubCuenta
        ...
        private void frmExportacion_Load(object sender, EventArgs e)
        {
            ...
            switch (Tipo)
            {
                ...
                default:
                    this.Text = "EXPORTACIÓN DE FACTURAS PROPIAS " + GloblaVar.VERSION;
                    GloblaVar.gUTIL.CartelTraza("EXPORTACIÓN DE FACTURAS PROPIAS PARA ContaPLUS " + GloblaVar.VERSION);
                    break;
            }
        ...
        private void button_Exportar_xBase_Click(object sender, EventArgs e)
        {
            ...
            //1.- se genera un array de objetos factura conteniendo los datos de las facturas que han de ir en el fichero
            //2.- Abrir fichero de diario que se va a crear
            //3.- Leer factura a Factura del array y grabar asiento correspondiente en el fichero
            //4.- Cerrar fichero de diario.
            //5.- Generar array con objetos cliente con los clientes dados de alta los ultimos 30 dias
            //6.- Abrir fichero subcuentas que vamos a crear
            //7.- Leer cliente a cliente del array y grabarlos en el fichero subcuentas
            //8.- Cerrar fichero subcuentas


            //Procediendo

            //1.- se genera un array de objetos factura conteniendo los datos de las facturas que han de ir en el fichero
            ArrayList Lista_Fras = new ArrayList();  //Lista_Fras con los objetos facturas seleccionados

            //2.- Preparar Lista de Facturas que va a grabarse

            for (int x = 0; x < dataGridView_Facturado.Rows.Count; x++)
            {
                if (dataGridView_Facturado.Rows[x].Cells[cSel].Value.ToString().ToLower() == "true")
                {
                    clase__FACTURA F = new clase__FACTURA();
                    F.Factura =Convert.ToInt32( dataGridView_Facturado.Rows[x].Cells[cFra ].Value);
                    F.Anyo = Convert.ToInt32(dataGridView_Facturado.Rows[x].Cells[cAño].Value);
                    F.Serie =dataGridView_Facturado.Rows[x].Cells[cSer ].Value.ToString();
                    F.DetCod  = Convert.ToInt32(dataGridView_Facturado.Rows[x].Cells[cDet ].Value);
                    F.DetNom  =dataGridView_Facturado.Rows[x].Cells[cDNm ].Value.ToString();
                    F.FechaEmision  = Convert.ToDateTime(dataGridView_Facturado.Rows[x].Cells[cFec ].Value);
                    F.ImpteFactura  = Convert.ToDecimal(dataGridView_Facturado.Rows[x].Cells[cImp ].Value);
                    F.ImptePendiente  = Convert.ToDecimal(dataGridView_Facturado.Rows[x].Cells[cPte].Value);
                    //F.F = Convert.ToInt32(dataGridView_Facturado.Rows[x].Cells[cSel ].Value);
                    F.IVA1  = Convert.ToDecimal(dataGridView_Facturado.Rows[x].Cells[cIVA ].Value);
                    F.RE1  = Convert.ToDecimal(dataGridView_Facturado.Rows[x].Cells[cReq].Value);
                    F.SubCuenta  =dataGridView_Facturado.Rows[x].Cells[cSbt ].Value.ToString();
                    Lista_Fras.Add(F);
                    GloblaVar.gUTIL.ATraza (gIdent +"Fra " + F.Factura.ToString() + "/" + F.Anyo + " a la Lista");
                    F = null;
                }   //if (dataGridView_Facturado.Rows[x].Cells[cSel].Value.ToString().ToLower() == "true")
                //Lista preparada


                //3.- Llamar a xBase_Diario para que genere el fichero y pasale la lista.


            }  //for (int x = 0; x < dataGridView_Facturado.Rows.Count; x++)


            //Creamos el fichero xBase que contendrá el archivo
            if (File.Exists("C:\\OREMAPE\\xDiario.dbf"))
            {
                PathDest = "C:\\OREMAPE\\CONTAPLUS\\";
                FDest = DateTime.Today.Year.ToString() + DateTime.Today.Month.ToString().PadLeft(2, '0') + DateTime.Today.Day.ToString().PadLeft(2, '0');
                FDest += "_XDIARIO.dbf";
                FDest = "xDiario.dbf";
                PathDest += FDest;
                if (File.Exists(PathDest))  //Si el Fichero existe hay que borrarlo
                {
                    File.Delete(PathDest);
                }
                File.Copy("C:\\OREMAPE\\xDiario.dbf", PathDest);
            }
            else
            {
                MessageBox.Show("No se se encontro el archivo", "Alerta", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                GloblaVar.gUTIL.ATraza("No se se encontro el archivo C:\\OREMAPE\xDiario.dbf");
            }



            //xBase_Diario(PathDest, Lista_Fras);
            xBase_Diario("C:\\OREMAPE\\CONTAPLUS\\xDiario.dbf", Lista_Fras);
        ...
        private void button_Exportar_Click(object sender, EventArgs e)
        {
            ...
            //Aquí guardamos el fichero DIARIO
            ...
            for (int x = 0; x < dataGridView_Facturado.Rows.Count; x++)
            {
                if (dataGridView_Facturado.Rows[x].Cells[8].Value.ToString().ToLower() == "true")
                {
                    ...
                    if (RE1 == "0.00")
                    {
                        ...
                        linea_iva += blanco.PadLeft(100, ' ') + blanco.PadLeft(50, ' ') + blanco.PadLeft(50, ' ') + blanco + uno.PadLeft(8, ' ') + blanco.PadLeft(40, ' ') + blanco.PadLeft(40, ' ') + "1" + NIF_Contrapartida.PadRight(15, ' ') + nombre_cliente.PadRight(40, ' ');
        ...
        private void Seguridad()
        {
            switch (frmPpal.USUARIO)
            {
                ...
                case "3":   //ENDUMAR
                    break;
                case "5":   //DIALPESCA
                    button_Exportar_xBase.Visible = true;
                    break;
                case "6":   //ABT
                    break;
                case "8":   //VALPEIX
        ...
        private void xBase_Diario(string Fichero,ArrayList ListaF)

            //Grabará en el fichero dado una lista de facturas con formato para contaplus. La lista son objetos de clase_FACTURA
        {
            //string sBase = "C:\\OREMAPE\\XDIARIO.dbf";
            string sSelect = "SELECT * FROM  " + FDest;
            string sConn;

            //sConn = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + System.IO.Path.GetDirectoryName(sBase) + ";Extended Properties=dBASE IV;";
            sConn = "Provider=Microsoft.ACE.OLEDB.12.0;Data Source=" + System.IO.Path.GetDirectoryName(Fichero) + ";Extended Properties=dBASE IV;";
            //sConn = "Provider= dBASE IV;Data Source=" + System.IO.Path.GetDirectoryName(sBase) + ";Extended Properties=dBASE IV;";
            ...
            using (System.Data.OleDb.OleDbConnection dbConn = new System.Data.OleDb.OleDbConnection(sConn))
            {
                try
                {
                    ...
                    clase__FACTURA F = new clase__FACTURA();
                    string insert = "";
                    //System.Data.OleDb.OleDbCommand cmd = new System.Data.OleDb.OleDbCommand("DELETE FROM " + PathDest, dbConn);
                    System.Data.OleDb.OleDbCommand cmd = new System.Data.OleDb.OleDbCommand("DELETE FROM " +FDest , dbConn);
                    cmd.ExecuteNonQuery();
                    int asiento = 1;
                    
                    for (int i=0;i<ListaF.Count;i++)
                    {
                        F = (clase__FACTURA )ListaF[i];
                        //                   SUBCTA                 CONTRA      EURODEBE    EUROHABER   BASEEURO
                        //Linea1 Asiento:   SubCta Cliente                          Total        0           BI
                        //Linea2 Asiento:   Cta. Ventas                             0           BI          BI
                        //Linea3 Asiento:   Cta. IVA              SubCta Clte       0           IVA         BI
                        //Linea4 Asiento:   Cta. Recargo                            0           REquiv      BI
                        //La linea 4 solamente se incorpora si se da el caso que la factura tiene recargo

                        //                   SUBCTA                 CONTRA      EURODEBE    EUROHABER   BASEEURO
                        //Linea1 Asiento:   SubCta Cliente                          Total        0           BI

                        insert = "INSERT INTO " +FDest +" ( ASIEN,FECHA,SUBCTA,CONTRA,PTADEBE,CONCEPTO,PTAHABER,FACTURA,BASEIMPO,IVA,RECEQUIV,NCASADO,TCASADO,TRANS,CAMBIO,DEBEME,HABERME,SERIE,IMPAUXME,MONEDAUSO,EURODEBE,EUROHABER,BASEEURO,NOCONV,NUMEROINV,L340)";
                        insert += " VALUES (";
                        insert += asiento.ToString() + ",";             //ASIENTO
                        insert += "'" +  F.FechaEmision.ToShortDateString()  + "',";                //FECHA
                        insert += "'" +F.SubCuenta  + "',";                                         //SUBCTA
                        insert += "'',";                                                            //CONTRA
                        insert += "0,";                                                             //PtaDebe
                        insert += "'Fra " + F.Factura.ToString()+" " +F.Serie.ToString() + "',";    //Concepto
                        insert += "0,";                                                             //PtaHaber
                        insert += F.Factura.ToString() + ",";                                       //Numero de Factura
                        insert += "0,";                                                             //BASEIMPO
                        insert += "0,";                                                             //IVA
                        insert += "0,";                                                             //RECARGO Equivalencia
                        insert += "0,";                                                             //NCASADO
                        ...
                        insert += "0,";                                                             //DEBEME
                        insert += "0,";                                                             //HABERME
                        insert += "'" + F.Serie.ToString().Substring(F.Serie.ToString().Length-1,1) + "',";      //SERIE
                        insert += "0,";                                                                         //IMPAUXME
                        insert += "'2',";                                                       //MONEDAUSO
                        insert += "" + F.ImpteFactura.ToString().Replace(",",".") + ",";        //EURODEBE
                        insert += "0,";                                 //EUROHABER
                        insert += "" + F.BI1.ToString().Replace(",", ".") + ",";                //BASEEURO
                        insert += "0,";                                                         //NOCONV
                        insert += "0,";                                                         //NUMEROINV
                        insert += "0";                                                          //L340
                        insert += ")";
                        ...
                        insert += "'2',";                               //MONEDAUSO
                        insert += "0,";                                 //EURODEBE
                        insert += "" +F.BI1.ToString().Replace(",", ".") + ",";           //EUROHABER
                        insert += "" + F.BI1.ToString().Replace(",", ".") + ",";          //BASEEURO
                        insert += "0,";                                 //NOCONV
                        insert += "0,";                                 //NUMEROINV
                        insert += "0";                                 //L340
                        ...
                        insert += " VALUES (";
                        insert += asiento.ToString() + ",";
                        insert += "'" + F.FechaEmision.ToShortDateString() + "',";
                        insert +="'" + SubCtaIVA  + "',";
                        insert += "'" +F.SubCuenta +"',";                                  //CONTRA
                        insert += "0,";                                             //PtaDebe
                        insert += "'Fra " + F.Factura.ToString() + " " + F.Serie.ToString() + "',";    //Concepto
                        insert += "0,";                                 //PtaHaber
                        insert += F.Factura.ToString() + ",";           //Numero de Factura
                        insert +=F.BI1.ToString().Replace(",",".")  +",";//BASEIMPO
                        insert += "10,";                                 //IVA (Habría de cambiarse cuando cambie el IVA o ajustarse )
                        insert += "0,";                                 //RECARGO Equival encia
                        insert += "0,";                                 //NCASADO
                        insert += "0,";                                 //TCASADO
                        insert += "0,";                                 //TRANS
                        insert += "0,";                                 //CAMBIO
                        ...
                        if (F.RE1>0)
                        {
                            insert = "INSERT INTO  " + FDest + " ( ASIEN,FECHA,SUBCTA,CONTRA,PTADEBE,CONCEPTO,PTAHABER,FACTURA,BASEIMPO,IVA,RECEQUIV,NCASADO,TCASADO,TRANS,CAMBIO,DEBEME,HABERME,SERIE,SUCURSAL,IMPAUXME,MONEDAUSO,EURODEBE,EUROHABER,BASEEURO,NOCONV,NUMEROINV,L340)";
                            insert += " VALUES (";
                            insert += asiento.ToString() + ",";
                            insert += "'" + F.FechaEmision + "',";
                            insert += SubCtaREQ  + ",";
                            insert += ",";                                  //CONTRA
                            insert += "0,";                                 //PtaDebe
                            insert += "'Fra " + F.Factura.ToString() + " " + F.Serie.ToString() + "',";    //Concepto
                            ...
                            insert += "0,";                                 //DEBEME
                            insert += "0";                                  //HABERME
                            insert += "'" + F.Serie.ToString() + "',";      //SERIE
                            insert += "0,";                                 //IMPAUXME
                            insert += "'2',";                               //MONEDAUSO
                            insert += "0,";                                 //EURODEBE
                            insert += "" + F.BI1.ToString() + ",";           //EUROHABER
                            insert += "" + F.BI1.ToString() + ",";          //BASEEURO
                            insert += "0,";                                 //NOCONV
                            insert += "0,";                                 //NUMEROINV
                            insert += "0,";                                 //L340
                            insert += ")";
